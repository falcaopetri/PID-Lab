...
var camStream = require("@camStream");
    ...
    io.on("connection", function(socket) {
        ...
        function disconnect(data, callback) {
            ...
            if (socket.connName) {
                app.pidController.disconnect(socket.id, socket.connName, function(err) {
                    ...
                    // TODO should check if the socket is really live streaming
                    logger.info('leaving room', "live_streaming");
                    socket.leave("live_streaming");
        ...
        socket.on("request_connection", function(data, fn) {
            ...
            app.pidController.connect(socket.id,
                data.connName,
                function(message) {
                    io.to(data.connName).emit("new_controller_data", {
                        message: message
                    });
                },
                function(err) {
        ...
        socket.on("request_live_streaming", function() {
            var updateFrame = function (buffer) {
                io.to("live_streaming").emit('update_frame', {
                    image: true,
                    buffer: buffer
                });
            }
            camStream.requestStream(socket.id, updateFrame);

            socket.join("live_streaming");
        });

        socket.on("stop_live_streaming", function() {
            camStream.stopStream(socket.id);
            socket.leave("live_streaming");
        });
